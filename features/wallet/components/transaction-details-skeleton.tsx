export default function TransactionDetailsSkeleton() {
  const rowWidths = ["w-40", "w-32", "w-52", "w-36", "w-28", "w-44", "w-48", "w-24"]

  return (
    <div className="w-full max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      {/* Header */}
      <div>
        <div className="w-40 h-5 bg-slate-200 rounded mb-6 animate-pulse"></div>
        <div className="w-72 h-8 bg-slate-200 rounded mb-2 animate-pulse"></div>
        <div className="w-52 h-4 bg-slate-200 rounded animate-pulse"></div>
      </div>

      <div className="bg-white rounded-xl border border-slate-200 p-6 sm:p-8">
        {/* Top Summary Section */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-4">
          {/* Amount Section */}
          <div className="flex flex-col items-center justify-center flex-1">
            <div className="w-14 h-14 rounded-full bg-slate-200 mb-4 animate-pulse"></div>
            <div className="w-20 h-6 rounded-full bg-slate-200 mb-3 animate-pulse"></div>
            <div className="w-32 h-8 bg-slate-200 rounded mb-2 animate-pulse"></div>
            <div className="w-24 h-5 bg-slate-200 rounded mb-3 animate-pulse"></div>
            <div className="w-20 h-6 bg-slate-200 rounded-full animate-pulse"></div>
          </div>

          <div className="hidden md:block w-px h-40 bg-slate-100"></div>

          {/* Date & Time Section */}
          <div className="flex flex-col items-center justify-center flex-1">
            <div className="w-12 h-12 rounded-full bg-slate-200 mb-3 animate-pulse"></div>
            <div className="w-20 h-4 bg-slate-200 rounded mb-2 animate-pulse"></div>
            <div className="w-24 h-5 bg-slate-200 rounded mb-1 animate-pulse"></div>
            <div className="w-16 h-5 bg-slate-200 rounded animate-pulse"></div>
          </div>

          <div className="hidden md:block w-px h-40 bg-slate-100"></div>

          {/* Reference ID Section */}
          <div className="flex flex-col items-center justify-center flex-1">
            <div className="w-12 h-12 rounded-full bg-slate-200 mb-3 animate-pulse"></div>
            <div className="w-24 h-4 bg-slate-200 rounded mb-2 animate-pulse"></div>
            <div className="w-36 h-5 bg-slate-200 rounded mb-3 animate-pulse"></div>
            <div className="w-6 h-6 bg-slate-200 rounded animate-pulse"></div>
          </div>

          <div className="hidden md:block w-px h-40 bg-slate-100"></div>

          {/* Ledger Code Section */}
          <div className="flex flex-col items-center justify-center flex-1">
            <div className="w-12 h-12 rounded-full bg-slate-200 mb-3 animate-pulse"></div>
            <div className="w-24 h-4 bg-slate-200 rounded mb-2 animate-pulse"></div>
            <div className="w-32 h-5 bg-slate-200 rounded mb-3 animate-pulse"></div>
            <div className="w-6 h-6 bg-slate-200 rounded animate-pulse"></div>
          </div>
        </div>

        <hr className="border-slate-200 my-8" />

        {/* Transaction Information Section */}
        <div>
          <div className="flex items-center gap-3 mb-6">
            <div className="w-8 h-8 rounded-full bg-slate-200 animate-pulse"></div>
            <div className="w-48 h-6 bg-slate-200 rounded animate-pulse"></div>
          </div>

          <div className="space-y-1">
            {rowWidths.map((valWidth, i) => (
              <div key={i} className="flex justify-between items-center py-4 border-b border-slate-100 last:border-0">
                <div className="w-32 h-5 bg-slate-200 rounded animate-pulse"></div>
                <div className="flex items-center gap-2">
                  <div className={`${valWidth} h-5 bg-slate-200 rounded animate-pulse`}></div>
                  {i < 3 && <div className="w-5 h-5 bg-slate-200 rounded animate-pulse"></div>}
                </div>
              </div>
            ))}
          </div>
        </div>

        <hr className="border-slate-200 my-8" />

        {/* Remarks Section */}
        <div>
          <div className="flex items-center gap-3 mb-4">
            <div className="w-8 h-8 rounded-full bg-slate-200 animate-pulse"></div>
            <div className="w-24 h-6 bg-slate-200 rounded animate-pulse"></div>
          </div>
          <div className="bg-slate-50 rounded-xl p-5 border border-slate-100 min-h-[100px] flex flex-col gap-2">
            <div className="w-[90%] h-4 bg-slate-200 rounded animate-pulse"></div>
            <div className="w-full h-4 bg-slate-200 rounded animate-pulse"></div>
            <div className="w-[75%] h-4 bg-slate-200 rounded animate-pulse"></div>
            <div className="w-[55%] h-4 bg-slate-200 rounded animate-pulse"></div>
          </div>
        </div>

        <hr className="border-slate-200 my-8" />

        {/* Need Help Section */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-slate-200 shrink-0 animate-pulse"></div>
            <div>
              <div className="w-28 h-5 bg-slate-200 rounded mb-1 animate-pulse"></div>
              <div className="w-72 h-4 bg-slate-200 rounded mb-1 animate-pulse"></div>
              <div className="w-48 h-4 bg-slate-200 rounded animate-pulse"></div>
            </div>
          </div>
          <div className="w-40 h-11 bg-slate-200 rounded-lg shrink-0 animate-pulse"></div>
        </div>
      </div>
    </div>
  )
}
